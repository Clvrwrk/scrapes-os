...
# Agentic OS - Recommended searchable memory setup.
#
# Usage:
...
usage() {
    cat <<'EOF'
Agentic OS searchable memory setup
...
choose_target() {
    ...
    echo "  brand context, and learnings. Agentic OS keeps markdown as the source"
...
confirm_setup() {
    ...
    echo "  - Index only Agentic OS memory files, not the full repo."
    case "$TARGET" in
...
ensure_memsearch_cli() {
    ...
    if ! command -v uv >/dev/null 2>&1; then
        ...
        echo "  Run Agentic OS dependency setup first:"
...
install_claude_plugin() {
    ...
    if ! claude plugin marketplace add zilliztech/memsearch --scope user; then
        warn "Could not add the MemSearch Claude Code marketplace automatically."
        echo "  Manual commands:"
        echo "    claude plugin marketplace add zilliztech/memsearch --scope user"
        echo "    claude plugin install memsearch --scope user"
        return 1
    fi

    if ! claude plugin install memsearch --scope user; then
...
echo "  and Agentic OS markdown memory remains the source of truth."
exit 1
