...
function Select-Target {
    ...
    Write-Host "  brand context, and learnings. Agentic OS keeps markdown as the source"
...
function Confirm-Setup {
    ...
    Write-Host "  - Index only Agentic OS memory files, not the full repo."
    switch ($Target) {
...
function Ensure-MemSearchCli {
    ...
    if (-not (Test-CommandAvailable -Name "uv")) {
        ...
        Write-Host "  Run Agentic OS dependency setup first:"
...
Write-Host "  and Agentic OS markdown memory remains the source of truth."
exit 1
